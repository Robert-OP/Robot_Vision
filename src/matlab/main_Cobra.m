...
for k = 1: length(allFigures)
    ...
    for i = 1:length(build)
        ...
        if onthefly == 0
            ...
            %Move robot to center (NOT ORIGEN)
            r.moveLinear(X_CENTER,Y_BUILD_NF(k),Z_CENTER,0,180,ANG_CENTER,VEL_MOVE)
            pause(0.1);
            
            %Construct:
            r.moveLinear(X_CENTER,Y_BUILD_NF(k),Z_BUILD_HOVER,0,180,ANG_CENTER,VEL_MOVE)
            pause(0.1);
            if i == 1
                r.moveLinear(X_CENTER,Y_BUILD_NF(k),Z1_BUILD_NF,0,180,ANG_CENTER,VEL_PICK)
                pause(0.1);
            elseif i == 2
                r.moveLinear(X_CENTER,Y_BUILD_NF(k),Z2_BUILD_NF,0,180,ANG_CENTER,VEL_PICK)
                pause(0.1);
            elseif i == 3
                r.moveLinear(X_CENTER,Y_BUILD_NF(k),Z3_BUILD_NF,0,180,ANG_CENTER,VEL_PICK)
                pause(0.1);
            end
        ...
        elseif onthefly == 1
            ...
            if i == length(build)
                ...
                r.closeGrapper
                pause(0.1);
                r.moveLinear(X_CENTER,Y_BUILD_NF(k),Z_CENTER,0,180,ANG_CENTER,VEL_PICK)
                pause(0.1);
